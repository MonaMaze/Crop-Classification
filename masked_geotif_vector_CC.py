...
ruv_shp = gpd.read_file('shapefile/ruv_summer_crop.shp')
with fiona.open("shapefile/ruv_summer_crop.shp", "r") as shapefile:
    features = [feature["geometry"] for feature in shapefile]

# Select the dates folders in LU folder
for date_folder in os.listdir('CC'):
    # Select the geotif images in each folder
    folder = os.path.join('CC', date_folder)
    for file_name in os.listdir(folder):
        ...
        res_path = os.path.join('masked_CC', date_folder)
...
with rasterio.open("images/CC/vector_shr.tif", "w", driver = "GTiff", crs = mas_shr.crs, transform = mas_shr.transform, dtype = rasterio.uint8, count = 1,
